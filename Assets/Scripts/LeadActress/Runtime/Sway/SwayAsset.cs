...
namespace LeadActress.Runtime.Sway {
    public static class SwayAsset {
        ...
        [NotNull]
        private static string GetParentBonePath([NotNull] string myPath) {
            var index = myPath.IndexOf('/');

            // strictly greater than
            if (index > 0) {
                return myPath.Substring(0, index);
            } else {
                return string.Empty;
            }
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static CapsuleAxis ParseCollidingAxis([NotNull] string str) {
            switch (str) {
                case "X":
                    return CapsuleAxis.X;
                case "Y":
                    return CapsuleAxis.Y;
                case "Z":
                    return CapsuleAxis.Z;
                default:
                    throw new ArgumentOutOfRangeException(nameof(str), str, null);
            }
        }
